...
namespace Kryptos.Hrms.API.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        [HttpPost]
        public ActionResult<Employee> CreateEmployee(CreateEmployee employee,double ctcpy)
        {
            try
            {

                //var createdByName = /*User.FindFirstValue(ClaimTypes.Name);*/"HR Admin";





                var createdEmployee = _provider.CreateEmployee(employee,ctcpy);
        ...
        public IActionResult GetAllManagers()
        {
            try
            {
                var managers = _context.Employees
                     .Where(x => x.IsActive == true && x.Role.Contains("Senior Manager"))
                     .Select(user => new
                     {
                         user.Id,
                         user.FirstName,
